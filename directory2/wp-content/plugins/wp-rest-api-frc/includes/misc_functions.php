...
BODY_ARR;
	...
	function frc_modified_items(){
		 ...
		 if('203.134.198.105' == $_SERVER["REMOTE_ADDR"]){
			  $query = "SELECT * FROM  `{$wpdb->prefix}postmeta` WHERE `meta_id` = 215419";
		 ...
		 $dbTimeAgo = $timeAgo  = mktime(date("H"), date("i"), date("s"), date("m")  , date("d")-8, date("Y"));
		 ...
		 $dbTimeNow = gmdate("Y-m-d H:i:s", time());
		 $postsTable = $wpdb->prefix."posts";
		 $postType = $isOldTheme ? 'ait-dir-item' : 'ait-item';
		 
		 $dbQry = "SELECT `$postsTable`.ID FROM `$postsTable`  WHERE (`$postsTable`.`post_modified_gmt` > '$dbTimeAgo' AND `{$wpdb->prefix}posts`.`post_modified_gmt` < '$dbTimeNow') AND `$postsTable`.`post_type` = '$postType' AND `$postsTable`.`post_status` IN('publish','future','draft','pending','trash')  ORDER BY `$postsTable`.`post_date` DESC";
		 ...
		 $args = array( 'post_type' => $postType, 'post__in' => $skippedPosts ); //temporary
		 */
		 ...
		 foreach ($items->posts as $key => $item) {
			  ...
			  if($item->post_status == 'publish'){
				   ...
				   if($isOldTheme){
						...
						$fullRating = '0';
						$ratingArr = array();
						$rating_mean_rounded = get_post_meta($item->ID, 'rating_mean_rounded');
						if(is_array($rating_mean_rounded)){
							 $fullRating = $ratingArr['rating_mean_rounded'] = $rating_mean_rounded[0];
							 $ratingArr['rating_mean'] = get_post_meta($item->ID, 'rating_mean')[0];
							 $ratingArr['rating_count'] = get_post_meta($item->ID, 'rating_count')[0];
						}
				   }
				   ...
				   while (($pos1 = strpos($item->post_content, $htmlTag[0], $pos1)) !== false) {
						...
						if($pos2 && $pos2 > $pos1){
							...
							$replaceWith = str_replace(array($htmlTag[0], $htmlTag[1]), array("<strong>","</strong>"), $tgtStr);
						}
				   ...
				   while (($pos1 = strpos($item->post_content, $htmlTag[2], $pos1)) !== false) {
						...
						if($pos2 && $pos2 > $pos1){
							$len = (int)$pos2 - (int)$pos1;
							$tgtStr1 = $tgtStr = substr($item->post_content, $pos1, $len);
							$replaceWith = str_replace(array($htmlTag[2], $htmlTag[3]), array("<strong>","</strong>"), $tgtStr);
						}
						$item->post_content = str_replace($tgtStr, $replaceWith, $item->post_content);
				   }
				   //<<<<<<<<<<<<<find tag to be replaced<<<<<<<<<<<<<
...
2. It is about the avatar on web side, as the new image uploaded from web, but didn’t sync to app side, vice versa. Please keep investigating this. Thanks.
   ...
   function add_data_2_post($dataArr, $campID, $campSiteTbl = 'camp_sites'){
		 ...
		 if(count($mainimage)){
		   //echo 'by yamini';die;
			  if(!empty($mainimage[0])){
			   ...
			   frc_delete_sqlite_photos($campID);// else block added on 25 may
			  }
		 }
		 ...
		 $dataArr['discount'] = '';
		 $dataArr['price'] = '';
		 //echo'<pre>';print_r($item->optionsDir);die;
		 if(is_array($item->optionsDir)){
		   //echo'<pre>';print_r($item->optionsDir);die;
			  $dataArr['address'] = $item->optionsDir['map']['address'];//code modified on 25 may
			  $dataArr['gps_lon'] = $item->optionsDir['map']['longitude'];//code modified on 25 may
			  $dataArr['gps_lat'] = $item->optionsDir['map']['latitude'];//code modified on 25 may
		...
		//echo'<pre>';print_r($item->optionsDir); die;
	   //--------Code added on 25 may------------------
	   $postMeta = get_post_meta($dataArr['site_id']);
	   ...
	   //print_r($itemData);die;
		//--------Code added on 25 may------------------
		 $features =  getFeaturesData1($itemData);//code modified on 25 may (replace $item->optionsDir to $itemData) 
		 ...
		 //echo '<pre>';print_r($fieldArr);die;//echo '<pre>';print_r($dataArr);die;
	...
	function update_sqlite_db($fieldArr, $tableName){
		 //echo "<pre>";print_r($fieldArr);die;//echo "</pre>";return;
	...
	//--------Code added on 25 may------------------
   function getFeaturesDataPlus1($itemData){
   ...
   function getFeaturesData1($itemData){
		 if(!is_array($itemData)){ return array();}
		 if(!is_old_theme()){return getFeaturesDataPlus1($itemData);}
		 //Feature 1
		 $features = array();
		 //Feature 1
...
$featureOptions[$key] = $fieldArr['label'];
   ...
   function frcCopyImages($photo, $isMain = 0, $itemID, $mobile_app_data = false){
		 ...
		 if( is_dir($itemAbsoluteDir) || is_dir($itemAbsDirUpdates) ){
		   //Partially For incremental updates
   ...
   //Code added on 25 May
   function frc_delete_sqlite_photos($postID){
	   $uploadDirArr = wp_upload_dir();
	   $mobDataDir = $uploadDirArr['basedir'].DS."mobile_app_data".DS."frc.db";
	   $dir = "sqlite:$mobDataDir";
	   $dbh = new PDO($dir) or die("cannot open the database");
	   $selQry = "SELECT `site_id` FROM `site_photos` WHERE `site_id` = $postID";
	   //echo "\n<br/>".$selQry;die;
	   $sth = $dbh->prepare($selQry);
	   $sth->execute();
	   $result = $sth->fetchAll(PDO::FETCH_ASSOC);
