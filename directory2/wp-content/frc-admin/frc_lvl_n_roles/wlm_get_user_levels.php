...
<?php
	...
	$FRC_Roles = array();
	$roleOptions = array(
							  'Directory' => array($wpdb->prefix.'options' , $wpdb->prefix.'user_roles'), //Field option_name = 'gwr_user_roles'
...
ROLE_STR;
	...
	$FRC_Levels = array();
	$lvlOptions = array(
							  'Directory' => array($wpdb->prefix.'wlm_options' , 'wpm_levels'), 
...
LVL_STR;
    ...
    $tableWLM_LvlArr = array($wpdb->prefix."{$wlmLvlTable}", "shop_{$wlmLvlTable}", "wp_{$wlmLvlTable}", "classified_{$wlmLvlTable}");
    $tableWLMUserOptionArr = array($wpdb->prefix."{$wlmUserOptionTbl}", "shop_{$wlmUserOptionTbl}", "wp_{$wlmUserOptionTbl}", "classified_{$wlmUserOptionTbl}");
...
SEARCH_FORM;
    ...
    if(isset($_REQUEST['update'])){
        $roleCapabilities = array($wpdb->prefix.'capabilities', 'classified_capabilities', 'shop_capabilities', 'wp_capabilities');
        $userID = (int)$_REQUEST['user_id'];
        //Start: Block for updating user roles
        $query = "SELECT * FROM `{$wpdb->prefix}usermeta` WHERE `user_id` = $userID AND `meta_key` = '{$wpdb->prefix}capabilities' ";
        $resultObj = $wpdb->get_results($query);
        ...
        if(count($resultObj) >= 1){
			...
			$updated = $wpdb->update( $wpdb->prefix.'usermeta',
									 array('meta_value' => $newUsrCapStr),
									 array('user_id' => $userID, 'meta_key' => "{$wpdb->prefix}capabilities") );
			foreach($roleCapabilities as $key => $capName){
				if($key == 0)continue;
				$query = "SELECT * FROM `{$wpdb->prefix}usermeta` WHERE `user_id` = $userID AND `meta_key` = '$capName' ";
				$resultObj = $wpdb->get_results($query);
				if(count($resultObj) >= 1){
					//update record
					$updated = $wpdb->update( "{$wpdb->prefix}usermeta",
					...
					//insert record
					$updated = $wpdb->insert( "{$wpdb->prefix}usermeta",
        ...
        if(!empty($diffArr)){
            $userID = (int)$_REQUEST['user_id'];
            $query = "SELECT `ID` FROM `{$wpdb->prefix}users` WHERE `ID` = '$userID'";
			$resultObj = $wpdb->get_results($query);
	...
	if(isset($_REQUEST['search'])){
		if(!empty($_REQUEST['login'])){
			...
			$query = "SELECT `ID` FROM `{$wpdb->prefix}users` WHERE LOWER(`user_login`) = '$userLogin'";
			$resultObj = $wpdb->get_results($query);
			...
			$query = "SELECT `ID` FROM `{$wpdb->prefix}users` WHERE `ID` = '$user_id'";
			$resultObj = $wpdb->get_results($query);
			if(count($resultObj) >= 1){
				$userID = $resultObj[0]->ID;
				$recFound = true;
	...
	if(is_plugin_active($wishlistPlugin) && $userID){
		...
		//Get User roles
		$query = "SELECT * FROM `{$wpdb->prefix}usermeta` WHERE `user_id` IN ('".implode("','", array($userID))."') AND  meta_key like '%_capabilities'";
		$resultObj = $wpdb->get_results($query);
		$capArr = array();
		if(count($resultObj)){
			foreach($resultObj as $capObj){
		...
		$dirRoles = array_keys($capArr["{$wpdb->prefix}capabilities"]);
