<html>
    <head>
        <title>FRC - delete image from other ITEM's</title>
...
?>
    <body>
        <h3>Delete Image From Other Items &raquo;</h3>
...
<?php
    if(isset($_REQUEST['delete-submit'])){
        if(array_key_exists("itemImgs", $_REQUEST) && is_array($_REQUEST['itemImgs']) && !empty($_REQUEST['itemImgs'])){
            if(!empty($_REQUEST['ids_for_copy_image'])){
                ...
                if($isOldTheme){
                    ...
                    $itemsFromWhereDeletedArr = explode(",", $_REQUEST['ids_for_copy_image']);
                    if(count($imgs2BeDeletedArr)){
                        $destinationUniqueArray = array();
                        foreach($itemsFromWhereDeletedArr as $key => $destinationId){
                            ...
                            if(!empty($destinationRes)){
                                ...
                                if(array_key_exists("gallery",$destinationDataArr) && is_array($destinationDataArr['gallery']) && !empty($destinationDataArr['gallery'])){
                                    foreach($destinationDataArr["gallery"] as $key => $destinationImageData){
                                        if(in_array($destinationImageData['image'],$imgs2BeDeletedArr)){
                                            continue;
                                        }
                                    ...
                                    $keY = 0;
                                    foreach($destinationUniqueArray as $title => $image){
...
HTML_FORM;
                ...
                if(!empty($imagesArr)){
                    ...
                    foreach($imagesArr as $key => $imageData){
                        ...
                        $imgRows.="<tr>
                        <td><a href='$imagePath' target='_blank'><img src='$imagePath' style='height:100px;width: 100px;' title='$imageTitle'/></a>&nbsp;Delete <input type='checkbox' name='itemImgs[]' value='$imagePath' /></td>
