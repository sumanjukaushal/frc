<?php
    //Script URL: http://freerangecamping.co.nz/directory/wp-content/manage_api_jobs/track_records_debug.php
    ...
    $published_posts = $count_posts->publish; //8521
    $offset = 100;
	...
	function get_root_category($campCategories = array()){
		...
		$rootCats = array();
		$rootCat = 9;	//default is Free
		if(count($campCategories) >= 1){
			$rootCat = $campCategories[0];
		}
		$bussinessCatArr = array_flip($businessCats);
		//frc_partner_cat_ids
		$campCatArr = array_flip($camp_cats);
		$partnerCatIds = frc_partner_cat_ids();
		//echo "<pre>frc_partner_cat_ids:";print_r($partnerCatIds);echo "</pre>";
		//echo "<pre>get_root_category:";print_r($campCategories);echo "</pre>";
   ...
   function frcExport($offset = 0){
		global $innerSeparator, $betSeparator;
		$partnerCatIds = frc_partner_cat_ids();
        ...
        $params = array(
                            ...
                            'post_status' => 'publish',
							'post__in' => array( 27390),//18891, 10530, 10531, 10546, 10699, 10916, 18884, 19056
                            'offset' => 0,	//$offset,
        ...
        foreach ($items->posts as $key => $item) {
			...
			//SELECT * FROM `camp_sites` WHERE TRIM(`discount`) != "" AND category_id IN('14', '155')
			...
			$itemArr['is_frc_partner'] = $is_frc_partner;	//echo "json_encode:".json_encode($itemCatArr);
			$rootCat = get_root_category($itemCatArr);		//echo "\n<br/>rootCat".
			...
			/*echo "\n<br/>Site_id:".$itemArr['site_id'];
			echo "\n<br/>Category_id:".$itemArr['site_category_id'];
			echo "\n<br/>cat ids:". $itemCatIds;
			echo "\n<br>-------------^^^^-----------------";
			continue;*/
			echo "<pre>";print_r($fieldArr);continue;//die;
			if(
