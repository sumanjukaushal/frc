<?php
    ...
    //>>>>>>>>>>>>> For Separate DB >>>>>>>>>>>
    echo "\n\n:Results from database freerang_premium<br/>\n";
    ...
    $postsTable = "wp_posts";
    $dbQry = "SELECT `ID` FROM `$postsTable`  WHERE (`$postsTable`.`post_modified_gmt` > '$dbTimeAgo' AND `gwr_posts`.`post_modified_gmt` < '$dbTimeNow')";
    echo "\n".$query = "SELECT * FROM `$postsTable` WHERE `post_type`='shop_order' AND `post_status` = 'wc-processing' AND (`post_modified_gmt` > '$dbTimeAgo' AND `post_modified_gmt` < '$dbTimeNow')";
    $result = $wpdb->get_results($query);//echo'<pre>';print_r($result);die;
        ...
        if(!empty($usrResult)){
            ...
            if(count($usrSelResult) >= 1){
                ...
                $dataArr[] = array(
                                 ...
                                 'order_total' => $order_total,
                                 'billing_shipping' => $billing_shipping,
                                 'login' => $usrResult[0]->user_login,
                                 'email' => $usrResult[0]->user_email,
                                );//print_r($dataArr);die;
            }
        }
    }
    ...
    foreach($dataArr as $sngDataArr){
        $user_id = (int)$sngDataArr['user_id'];
        $userSelQry = "SELECT * FROM `frc_premium_users` WHERE `user_id` = $user_id";
        $usrSelResult = $wpdb->get_results($userSelQry);
        if(count($usrSelResult) >= 1){
                ;//do nothing
    ...
    //<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    echo "\n\n<br/>$recordCounter new records added";
?>
