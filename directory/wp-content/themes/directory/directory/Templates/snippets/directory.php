...
function getItems($category = 0, $location = 0, $search = '', $radius = array()) {
	...
	**/
	...
	if(is_array($location) && count($location) >= 1){
		...
		if(empty($locRadius)){$locRadius = 20;} //Asked by Rob on 7th Feb, message_id =
		$term = get_term( $location, 'ait-dir-item-location');
	}
	...
	if(isset($_REQUEST['test_query'])){
		echo "<pre>URL $search";echo "#### $locRadius ######</pre>";
	}
	...
	if(isset($term) && is_object($term)){
		...
		$searchURL = "http://maps.google.com/maps/api/geocode/json?address={$prepAddr}&sensor=false";
		...
		if(isset($_REQUEST['test_query'])){
			echo "<pre>URL if";print_r($searchURL);echo "##########</pre>";
		}
	}elseif(!empty($search)){
		$locationName = $search;
		$address = "{$locationName}, Australia";
		...
		//if(!empty($locRadius) && $locFound){
		if($locFound){
			if(empty($locRadius))$locRadius = 20;
			$radius = array($locRadius, $lat,$long);
		}else{
			$radius = array();
		}
		if(isset($_REQUEST['test_query'])){
			//echo "<pre>URL else";print_r($radius);echo "##########</pre>";
		}
	}
	...
	if(($_REQUEST['category'] == 0) && ($_REQUEST['location'] == 0) && !isset($_REQUEST['categories'])){
		$category = array(9,14,155,12,20,5,6,5,8,2087,210, 10);
	}
	//-----hack added on Feb 12 for case when we are searching only custom location--------
	$taxquery['relation'] = 'AND';
	if(strpos($category, ",")){
	...
	if(isset($_REQUEST['test_query']) || '124.253.66.222' == $ipAddress){
			//$params['author'] = '1';
           //echo "<pre>---params"; print_r($params);echo "---</pre>";
    }
	if(isset($_REQUEST['test_query'])){
		//die;
		echo "<pre>---";print_r($radius);echo "##########</pre>";
		echo "<pre>---";print_r($output->status);echo "##########</pre>";
		//die;
	}
	global $wpdb;
	$itemsQuery = new WP_Query();
	...
	if($category == 0 && $location == 0 && empty($_REQUEST['search'])){
		...
		if(isset($_REQUEST['test_query'])){
			echo "<pre>--global if-";print_r($radius);echo "##########</pre>";
		}
		...
		if(isset($_REQUEST['test_query'])){
			echo "<pre>--global else-";print_r($params);echo "##########</pre>";
		}
		if(isset($_REQUEST['rasu'])){
			echo "<pre>else";
			print_r($_REQUEST);
			...
			echo "Last SQL-Query: {$itemsQuery->request}<br/>\n\n</br/>";
			echo $userID = get_current_user_id();
			echo "<pre>Tax Query:"; print_r($taxquery);echo "</pre>";
			echo "<pre>Params:"; print_r($params);echo "</pre>";
		...
		if(isset($_REQUEST['test_query'])){
			echo "<pre>--global else-";print_r($radius);echo "##########</pre>";
		}
		...
		if(isset($_REQUEST['test_query'])){
			echo "<pre>---param";print_r($items);echo "##########</pre>";
		}
	...
	// add item details
	//failing for 549 so reducing to 198
	$counter = 0;
	...
	foreach ($items as $key => $item) {
		$counter++;
		//if($counter >= 100){unset($items[$key]);continue;}
		// options
		$item->optionsDir = get_post_meta($item->ID, '_ait-dir-item', true);
		...
		if(isset($_REQUEST['test_query'])){
			//echo "<pre>";
			//echo "\nmeta tags for post id:".$item->ID."\n";
			//print_r($item->optionsDir);
			//if($featuresMatching){
			//	echo "all features: ".implode(",",$_REQUEST['feature'])." qualifying";
			//}else{
			//	echo "No feature is: ".implode(",",$_REQUEST['feature'])." qualifying";
			//}
			//echo "</pre>";				
		}
