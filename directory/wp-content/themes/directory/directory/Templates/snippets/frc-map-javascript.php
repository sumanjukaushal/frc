...
{if !empty($items)}
	{foreach $items as $item}
		...
		(!empty($item->optionsDir['offer Offers Apply'])) || 
...
?>
			...
			{if (!empty($item->optionsDir['noCamperTrailers']))}				
				{var $imageSrc = "$url/uploads/sites/3/2014/04/facility_icons/30x30/jpg/";}
			...
			{if (!empty($item->optionsDir['pubs']))}				
				{var $imageSrc = "$url/uploads/sites/3/2014/04/facility_icons/30x30/jpg/pubs";}
		...
		{else}
			...
			dynamicImageContent = '<div class="marker-content{ifset $item->thumbnailDir} with-image" style="position: relative;"><img src="{var $RealThumbnailUrl = getRealThumbnailUrl($item->thumbnailDir)}{thumbnailResize $RealThumbnailUrl, w => 120, h => 160}" alt="">{else}">{/ifset}';
		{/if}
		{/if}
						...
						data: 	'<div class="marker-holder">'+
									imageArray[{$item->ID}]+
										'<div class="map-item-info" style="padding:9px;">'+
											...
											'<a target={$targetLnk} href="{!$item->link}{!$appendToURL}" class="more-button">' + {__ 'VIEW MORE'} + '</a>'+
