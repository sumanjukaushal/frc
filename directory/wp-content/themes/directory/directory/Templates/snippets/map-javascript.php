...
?>
			{if (!empty($item->optionsDir['toilets']))}
				...
				<?php $imgContent.="<img width=\"30\" height=\"30\" src={$imageSrc} title = {__ 'Toilets Available'} alt = {__ 'Toilets Available'} />";?>
			{/if}
			...
			{if (!empty($item->optionsDir['offer Offers Apply']))}				
				...
				imagesContent = imagesContent + '<img width="30" height="30" src={$imageSrc} title = {__ ''} alt = {__ ''} />';
			{/if}
			...
			{if (!empty($item->optionsDir['pubs']))}				
				{var $imageSrc = "$url/uploads/sites/3/2014/04/facility_icons/30x30/jpg/pubs/pubs.png";}
			...
			{ifset $item->thumbnailDir}
				{var $RealThumbnailUrl = getRealThumbnailUrl($item->thumbnailDir)}
				var feaImgURL = '{thumbnailResize $RealThumbnailUrl, w => 120, h => 160}';
			{/ifset}
			dynamicImageContentFeatured = '<div class="marker-content{ifset $item->thumbnailDir} with-image" style="position: relative;"><img src="{var $RealThumbnailUrl = getRealThumbnailUrl($item->thumbnailDir)}{thumbnailResize $RealThumbnailUrl, w => 120, h => 160}" alt="">{else}">{/ifset}';
			
			if(imagesContent.trim() == ""){
				dynamicImageContent = dynamicImageContentFeatured;
				console.log('Item id:{$item->ID}: '+ dynamicImageContent);
			}else{
				console.log('Item id:{$item->ID}: '+ dynamicImageContent);
			}
