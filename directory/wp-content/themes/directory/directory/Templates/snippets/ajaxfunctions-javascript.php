<?php
    ...
    $isMyIP = isRemoteAddress(array('124.253.10.89', '149.135.34.230'));
?>
// loading spinner
var opts = {
...
function custom_ajaxGetMarkers(ajax,geoloc)
{
    ...
    //-------Start: form submission without ajax-----------
<?php
    if( true ){//$isMyIP || true
        $siteURL = site_url();
        echo $hashJS = <<< HASH_JS
        ...
        var query = locHref.split('#');
        console.log("Search URL:"+query);
        var urlParams = new URLSearchParams(window.location.href);
        console.log(urlParams.has('iframe'));
        console.log("ifrmae = "+urlParams.get('iframe'));
        var locRad = parseInt(locationRadius);
        var siteURL = '$siteURL?dir-search=yes&iframe=1&s=';
        //alert(siteURL);
        if(search != ""){
            siteURL = siteURL + "&search="+search;
        }
        
        if(radius != ""){
            siteURL = siteURL + "&loc_radius="+radius;
        }else if(locRadius != ""){
            siteURL = siteURL + "&loc_radius="+locRadius;
        }
        if(featureIDs != "" && featureIDs != null){
            siteURL = siteURL + "&features="+featureIDs;
        }
        if(location != ""){
            siteURL = siteURL + "&locations="+location;
        }
        if(category != ""){
            siteURL = siteURL + "&categories="+category;
        }else{
            ;//siteURL = siteURL + "&categories=9,30,14,155,12,20,6,2096,5,7,8,10,106,15,17,4,2797,107,2088";
        }
        //alert(siteURL);
        if((undefined !== query[1] && query[1] == "iframe=1") || true){
            //jQuery.unblockUI();
            console.log("href:"+siteURL);
            window.location.href = siteURL;
            return;
        }
        if(urlParams.has('iframe')){
            //jQuery.unblockUI();
            console.log("anchor:"+siteURL);
            window.location.href = siteURL;
            return;
        }
HASH_JS;
...
?>
    //-------End: form submission without ajax-------------
    // get items from ajax
    $.post(
			...
			events: {
				click: function(marker, event, context){
					...
					infobox.draw = function() {
						...
						//jQuery(infobox.div_).fadeIn('slow'); //Fri 9th Jun, 2017
					}
					*/
					//------------------------
					infobox.setContent(context.data);
					//infobox.open(map,marker);//.delay( 800 ).fadeIn( 400 );
					infobox.open(map,marker);//.fadeIn( 400 ); //Fri 9th Jun, 2017
