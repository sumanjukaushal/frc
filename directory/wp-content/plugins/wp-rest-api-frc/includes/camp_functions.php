...
=================
	 ...
	 /*
		  ...
		  _ait-item_item-extension	|										|
		  ait-rating				|										|	Item Rating
		  --------------------------------------------------------------------------------------
		  SELECT * FROM  `gwr_postmeta` WHERE `meta_key` =  '_ait_dir_gallery' order by `post_id` desc
	 */
	 //3: Get camping site by id - subpart
	 function frc_item_details($itemID, $postObj, $lat = null, $lon = null, $distance = null){
		  ...
		  $query = "SELECT * FROM  `gwr_postmeta` WHERE `post_id` = $itemID AND  `meta_key` =  '_ait_dir_gallery'";//25761
		  ...
		  $ratingQuery = "SELECT P.`post_date` , P.`post_content` , P.`post_title`, `P`.`post_author` , M.`post_id` FROM  `gwr_posts` AS P,  `gwr_postmeta` AS M WHERE  `M`.`meta_key` =  'post_id' AND  `M`.`meta_value` = $itemID AND  `M`.`post_id` =  `P`.`ID` AND `P`.`post_status` = 'publish'";
		  ...
		  $siteRating = array();
		  if(count($ratingIDs)){
			   $ratingQry = "SELECT * FROM `gwr_postmeta` WHERE `post_id` IN('".implode("','",$ratingIDs)."') AND `meta_key` = 'rating_mean'";
		  ...
		  if(is_array($postMeta) && (array_key_exists('_ait-item_item-data', $postMeta) || array_key_exists('_ait-dir-item', $postMeta))){
			   if(array_key_exists('_ait-item_item-data', $postMeta)){
			   ...
			   $item['suburb'] = ""; //Not Implemented
			   $item['state'] = ""; //Not Implemented
			   $item['suburb'] = array_key_exists('wlm_city',$itemData) ? $itemData['wlm_city']: ''; //Not Implemented
			   $item['state'] = array_key_exists('wlm_state',$itemData) ? $itemData['wlm_state']: ''; //Not Implemented
			   $item['post_code'] = array_key_exists('wlm_zip',$itemData) ? $itemData['wlm_zip']: ''; //Not Implemented
