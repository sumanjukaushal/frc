<?php
	 ...
	 function remove_bookmark($user_id, $post_id, $rCollectID){
		  ...
		  if($_SERVER['REMOTE_ADDR'] == '122.173.46.89'){
			   $bookmarkQry = "SELECT * FROM `gwr_usermeta` WHERE `meta_key`='_wpb_bookmarks' and user_id = '$user_id'";
			   ...
			   $collectionQry = "SELECT * FROM `gwr_usermeta` WHERE `meta_key`='_wpb_collections' and user_id = '$user_id'";
		  ...
		  $query = "SELECT * FROM `gwr_usermeta` WHERE `meta_key`='_wpb_collections' and user_id = '$user_id'";
		  ...
		  if(is_array($collectionMeta) and count($collectionMeta) >= 1){
			   ...
			   foreach($collectionMetaData as $collectID => $siteIDsArr){
					...
					if($collectID == $rCollectID){
						 ...
						 $collectQuery = "UPDATE `gwr_usermeta` SET `meta_value` = '$collectSerialized' WHERE `umeta_id` = '$uMetaID'";
			   ...
			   if($sitePresentInOtherCollections){
					...
					/*$query = "SELECT * FROM `gwr_usermeta` WHERE `meta_key`='_wpb_bookmarks' and user_id = '$user_id'";
					...
					if(is_array($bookmarkMeta) && count($bookmarkMeta) >= 1){
						 ...
						 print_r($dataArr);
						 $bookmarkQuery = "UPDATE `gwr_usermeta` SET `meta_value` = '$bookmarkSerialized' WHERE `umeta_id` = '$uMetaID'";
						 ...
						 echo $wpdb->last_query;
						 update_post_meta($post_id , '_wpb_post_bookmark_count' , get_post_meta($post_id , '_wpb_post_bookmark_count' , true)-1);
						 $bookmarkQry = "SELECT * FROM `gwr_usermeta` WHERE `meta_key`='_wpb_collections' and user_id = '$user_id'";
						 ...
						 echo $bookmarkQuery = "UPDATE `gwr_usermeta` SET `meta_value` = '$bookmarkData' WHERE `umeta_id` = '$bookmarkKey'";
						 $bookmarkResult = $wpdb->update(
															'gwr_usermeta',
